     ...
     implícit amb la nota més gran */
{
    ...
    for(int i = 0; i < nest; ++i) {
        if (vest[i].te_nota()) {
            if (imax == -1) imax = i;
            else if (vest[imax].consultar_nota() < vest[i].consultar_nota()) imax = i;
        }
    }
}
...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
  ...
  if (est.te_nota()) {
        if (imax == -1) imax = i+1;
            ...
            if(vest[imax].consultar_nota() < est.consultar_nota()) imax = i +1;
            else if (est.consultar_nota() == vest[imax].consultar_nota() and imax > i+1) imax = i+1;
        }
  }
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  if(imax==-1){
      vest[i]=est;
      if(est.te_nota()) imax = i;
  }
  else{
      Estudiant ori = vest[i];
      vest[i]=est;
      if(i== imax){
          if(not est.te_nota() or (est.consultar_nota() < ori.consultar_nota()))
              recalcular_posicio_imax();
      }
      else{
          if(est.te_nota() and (est.consultar_nota() > vest[imax].consultar_nota())) imax = i;
          else if (est.te_nota() and (est.consultar_nota() == vest[imax].consultar_nota()) and i<imax) imax=i;
      }
  }
}
...
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est) {
  ...
  if(imax==-1){
        vest[i-1]=est;
        if(est.te_nota()) imax = i-1;
    }
    else{
        Estudiant ori = vest[i-1];
        vest[i-1]=est;
        if(i-1 == imax){
            if(not est.te_nota() or (est.consultar_nota() < ori.consultar_nota()))
                recalcular_posicio_imax();
            ...
            if(est.te_nota() and (est.consultar_nota() > vest[imax].consultar_nota()))
                imax = i-1;
            else if (est.te_nota() and (est.consultar_nota() == vest[imax].consultar_nota()) and i-1<imax) imax=i-1;
        }
    }
}
